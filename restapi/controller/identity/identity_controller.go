...
const (
	...
	getSnarkDataURL = operationID + "/snark_data"
...
func (c *Controller) getSnarkData(rw http.ResponseWriter, req *http.Request) {
	// logger.Debugf("Querying subjects")

	var request identityModel.GetSnarkDataRequest
	...
	Openfile.Seek(0, 0)
	io.Copy(rw, Openfile) //'Copy' the file to the client

	response := identityModel.GetSnarkDataResponse{
		Results: "",
	}
...
func (c *Controller) registerHandler() {
	...
	c.handlers = []controller.Handler{
		controller.NewHTTPHandler(getSnarkDataURL, http.MethodGet, c.getSnarkData),
